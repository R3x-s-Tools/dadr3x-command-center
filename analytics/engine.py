...
class AnalyticsEngine:
    ...
    def build_contextual_moments(self, events: list[Any], chat_history: list[dict], twitch_events: list[dict]) -> list[dict]:
        moments = []
        for event in events:
            ...
            human = [m for m in nearby_chat if not is_bot(str(m.get("username", "")))]
            bots = [m for m in nearby_chat if is_bot(str(m.get("username", "")))]
            nearby_events = self._nearby(twitch_events, event_ts)

            moments.append(MomentContext(
                stream_time=event_dict.get("stream_time", "?"),
                event_type=event_type,
                score=self._clip_score(event_dict, human, nearby_events),
                reason=event_dict.get("reason", ""),
                likely_cause=self._likely_cause(event_type, human, bots, nearby_events),
                human_chat_nearby=human[-8:],
                bot_chat_nearby=bots[-5:],
    ...
    def stream_score(self, summary: dict) -> dict:
        ...
        avg = float(viewers.get("average_viewers", 0) or 0)
        human_msgs = int(summary.get("total_chat_messages", 0) or 0)
        unique = int(summary.get("unique_chatters", 0) or 0)
        moments = summary.get("contextual_moments", [])

        viewer_score = min(100, int((avg * 12) + (peak * 4)))
        chat_score = min(100, int((human_msgs * 1.5) + (unique * 10)))
        clip_score = min(100, int(max([m.get("score", 0) for m in moments], default=0) * 10))
        technical_score = 100
        overall = int(viewer_score * 0.30 + chat_score * 0.25 + clip_score * 0.25 + technical_score * 0.20)
        return {"overall": overall, "viewer_retention": viewer_score, "chat_engagement": chat_score, "clip_potential": clip_score, "technical": technical_score}
    ...
    def _nearby(self, items: list[dict], timestamp_epoch: float) -> list[dict]:
        if not timestamp_epoch:
            return []
        return [item for item in items if abs(float(item.get("timestamp_epoch", 0) or 0) - timestamp_epoch) <= self.chat_window_seconds]

    def _likely_cause(self, event_type: str, human: list[dict], bots: list[dict], twitch_events: list[dict]) -> str:
        text = " ".join([str(m.get("message", "")) for m in human + bots] + [str(e.get("message", "")) for e in twitch_events]).lower()
    ...
    def _clip_score(self, event_dict: dict, human: list[dict], twitch_events: list[dict]) -> int:
        ...
        if len(human) >= 5:
            ...
            score += 1
        event_text = " ".join(str(e.get("message", "")) for e in twitch_events).lower()
